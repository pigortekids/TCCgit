...
def rodar_1dia(precos, custo, dia):
    ...
    for step in range( steps[dia_para_rodar - 1], steps[dia_para_rodar] ):  #roda os dados
        ...
        linhaAdicionar = {'dt':dt, 'preco':arquivo.iloc[step, 1], 'acao':acao, 'qnt_contratos':ncont,
    ...
    if reward > 0:
        global qnt_dias_positivos
        qnt_dias_positivos += 1
    else:
        global qnt_dias_negativos
        qnt_dias_negativos += 1

    step_final = steps[dia_para_rodar] - 1
    dt = arquivo.iloc[step_final, 0] + " 17:55:00"
    linhaAdicionar = {'dt':dt, 'preco':arquivo.iloc[step_final, 1], 'acao':-ncont, 'qnt_contratos':0,
                          'carteira':round(reward, 2), 'preco_ant':arquivo.iloc[step_final - 1, 1], 
                          'preco_pon':arquivo.iloc[step_final]['preco_pon'],
                          'qnt_soma':arquivo.iloc[step_final]['qnt_soma'],
