...
data_directory = directory + "/trading_agent/data"

# Hardware Parameters
CPU = True  # Selection between CPU or GPU
CPU_cores = 2  # If CPU, how many cores
GPU_mem_use = 0.8  # In both cases the GPU mem is going to be used, choose fraction to use

# Data Parameters
train_data = data_directory + '/train_data.npy'  # path to training data
MAX_DATA_SIZE = 1200000  # Maximum size of data
...
TEST_EPOCHS = 20  # How many test runs / epochs
TEST_POINTS = [0]  # From which point in the time series to start in each epoch
TEST_STEPS = 16000  # For how many points to run the epoch

# Validation Data
VALIDATE = True  # Use a validation set if available
VAL_DATA = data_directory + '/validation_data.npy'  # path to validation data set
VAL_SIZE = 627158  # Set the size of the validation data you want to use
TEST_EPOCHS_GEN = 10  # How many epochs for validation
TEST_STEPS_GEN = 16000  # How many steps in each epoch for validation

# Initialize random starts within the validation data
VAL_STARTS = np.random.randint(low=0, high=VAL_SIZE-TEST_STEPS_GEN-1, size=TEST_EPOCHS_GEN)
...
FOLDER = METHOD + "_e=" + str(EPOCHS) + "_s=" + str(STEPS) + "_w=" + str(WINDOW_LENGTH) + "_" + DATE
